...
- **WHEN** a user adds a display-count entry under `pins` in the configuration file, listing one or more rules each with an application bundle identifier, a title pattern, and a target `ordinal`
...
- **WHEN** a user adds a display-count entry under `default_spaces` in the configuration file, listing one or more rules each with an application bundle identifier and a target `ordinal`
- **THEN** those rules are available to `mumu restore` and `mumu show` for that display count
...
- **WHEN** a user runs a `mumu` command and the configuration file's `pins` setting contains a rule missing its title pattern or with a non-positive `ordinal`
- **THEN** the system reports a clear error identifying the configuration file path and the offending rule, naming the field as `ordinal`, and makes no changes
...
- **WHEN** a user runs a `mumu` command and the configuration file's `default_spaces` setting contains a rule missing its application bundle identifier or with a non-positive `ordinal`
- **THEN** the system reports a clear error identifying the configuration file path and the offending rule, naming the field as `ordinal`, and makes no changes
