...
- **Needs both Accessibility and Screen Recording permissions** — Screen Recording is needed to read window titles reliably for restore matching; see [`mumu status`](#mumu-status). If mumu's own check thinks either is missing, `save`/`restore` print a warning and proceed anyway rather than stopping (that check can be wrong on some machines — see [TROUBLESHOOTING.md](TROUBLESHOOTING.md)); a genuinely missing permission then shows up as an actual failure (no windows captured, or entries skipped) rather than a separate blocking error.
